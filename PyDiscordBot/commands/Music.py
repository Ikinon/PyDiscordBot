...
class Music(commands.Cog):
    ...
    async def pause(self, ctx):
        ...
        if player.is_paused:
            ...
            return await MessagingUtils.send_embed_command_success(ctx, "", "Player Unpaused")
        await player.set_pause(True)
        await MessagingUtils.send_embed_command_success(ctx, "", "Paused playback")

    @commands.command()
    async def play(self, ctx, *, query: str):
        """Plays a track from a query"""
        tracks = await self.wavelink.get_tracks(f'{query}')

        if not tracks:
            return await MessagingUtils.send_embed_command_fail(ctx, "", "Could not find any songs with that query")

        player = self.wavelink.get_player(ctx.guild.id)
        if not player.is_connected:
            await ctx.invoke(self._join)

        await MessagingUtils.send_embed_command_success(ctx, "", f"Added {str(tracks[0])} to queue")
    ...
    async def playing(self, ctx):
        ...
        if not song:
            return await MessagingUtils.send_embed_command_info(ctx, "", "Not currently playing anything")
        embed = MessagingUtils.embed_command_info(ctx, f"Current track: {str(song)}", "")
    ...
    async def search(self, ctx, *, query: str):
        ...
        embed = MessagingUtils.embed_command_info(ctx, f"Found songs with query {query}", "")
        max = 5
        if len(tracks) < 5:
            max = len(tracks)
        ...
        if not resp:
            return await msg.edit(embed=MessagingUtils.embed_command_success(ctx, "", f"Search response timeout"))
        await (self.wavelink.get_player(ctx.guild.id)).play(tracks[resp - 1])
        await msg.edit(
            embed=MessagingUtils.embed_command_success(ctx, "", f"Added {tracks[resp - 1]} to the queue"))

    @commands.command()
    async def stop(self, ctx):
        """Stops the player and clears the queue"""
        await (self.wavelink.get_player(ctx.guild.id)).destroy()
        await MessagingUtils.send_embed_command_success(ctx, "", "Stopped music")
    ...
    async def _leave(self, ctx):
        ...
        if channel is None:
            return await MessagingUtils.send_embed_command_warning(ctx, "", "I am not connected to a voice channel")
        await player.disconnect()
        await MessagingUtils.send_embed_command_success(ctx, "", f"Disconnected from voice channel {channel.name}")
    ...
    async def _volume(self, ctx, volume: int = None):
        player = self.wavelink.get_player(ctx.guild.id)
        if not volume:
            return await MessagingUtils.send_embed_command_info(ctx, "", f"Volume is currently at {player.volume}")
        if volume > 1000 or volume < 0:
            return await MessagingUtils.send_embed_command_warning(ctx, "", "Volume must be between 0 and 1000")

        await player.set_volume(volume)
        await MessagingUtils.send_embed_command_success(ctx, "", f"Adjusting player volume to {volume}.")
